...
public class ShopHistoryAdapter extends BaseAdapter {
    ...
    public ShopHistoryAdapter(Context context) {
        ...
        FloozRestClient.getInstance().shopHistory( new FloozHttpResponseHandler() {
            @Override
    ...
    public View getView(int i, View convertView, ViewGroup parent) {
        ...
        if (currentItem != null) {
            ...
            if (convertView == null || convertView.getTag() == null || !(convertView.getTag() instanceof ViewHolder)) {
                ...
                convertView = inflater.inflate(R.layout.shop_history_cell, parent, false);

                viewHolder.name = (TextView) convertView.findViewById(R.id.shop_history_cell_name);
                viewHolder.date = (TextView) convertView.findViewById(R.id.shop_history_cell_date);
                viewHolder.amount = (TextView) convertView.findViewById(R.id.shop_history_cell_amount);
            ...
            if (currentItem.has("code")) {
                if (currentItem.opt("code") instanceof String) {
                    viewHolder.name.setText(currentItem.optJSONObject("type").optString("name"));
                } else if (currentItem.opt("code") instanceof JSONArray) {
                    if (currentItem.optJSONArray("code").length() > 1) {
                        viewHolder.name.setText(currentItem.optJSONObject("type").optString("name") + " x " + currentItem.optJSONArray("code").length());
                    } else {
                        viewHolder.name.setText(currentItem.optJSONObject("type").optString("name"));
                    }
                }
    ...
    public void loadNextPage() {
        if (!isLoadingNext && nextURL != null && !nextURL.isEmpty()) {
            isLoadingNext = true;
            FloozRestClient.getInstance().shopHistoryNextPage(nextURL, new FloozHttpResponseHandler() {
                @Override
                public void success(Object response) {
                    Map<String, Object> ret = (Map<String, Object>) response;
